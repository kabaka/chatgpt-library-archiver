...
def _generate_html(page_items: List[Dict], page_num: int, total_pages: int) -> str:
    html = [
        ...
        "<style>",
        "body { font-family: sans-serif; margin: 20px; background: white; color: black; transition: background 0.3s, color 0.3s; }",
        ...
        ".image-card { border: 1px solid #ccc; padding: 8px; border-radius: 8px; font-size: 0.85em; background: white; }",
        ...
        ".toggle { background: #ddd; border-radius: 5px; padding: 6px 10px; cursor: pointer; font-size: 0.9em; }",
        "body.dark .toggle { background: #555; color: white; }",
        ".size-select { background: #ddd; border-radius: 5px; padding: 6px 10px; cursor: pointer; font-size: 0.9em; }",
    ...
    for item in page_items:
        ...
        html.extend([
            ...
            f"    {created}<br>",
            f"    Tags: {tags}<br>",
            f"    <a href=\"{item.get('conversation_link', '#')}\" target=\"_blank\">View conversation</a>",
            "  </div>",
            "</div>",
        ])
...
def _generate_index(num_pages: int) -> str:
    lines = [
        ...
        "<style>body { font-family: sans-serif; margin: 30px; } a { text-decoration: none; }</style>",
        "</head>",
        "<body>",
        "<h1>Image Gallery</h1>",
        "<ul>",
...
def generate_gallery(gallery_root: str = "gallery", images_per_page: int = 500) -> int:
    ...
    pages = [items[i:i + images_per_page] for i in range(0, len(items), images_per_page)]

    for idx, page in enumerate(pages, 1):
        html = _generate_html(page, idx, len(pages))
        with open(os.path.join(gallery_root, f"page_{idx}.html"), "w", encoding="utf-8") as f:
            f.write(html)
